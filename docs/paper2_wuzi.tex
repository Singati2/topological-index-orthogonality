...
% Paper 2: The Wuzi Index Family
% Compile with: pdflatex paper2_wuzi.tex (twice for cross-refs)
%
...
\]
The family is a strict generalization of several classical
...
$r$-regular graphs). Sharp bracket bounds in terms of the
edge-contribution function are established. Further bounds in
terms of order $n$, size $m$, and degree extremes
$(\Delta,\delta)$, as well as in terms of classical degree-based
indices, are stated as theorem skeletons whose detailed
derivations are reserved for the companion mathematical work.
Extremal-graph behavior among trees, unicyclic graphs, and
bicyclic graphs is conjectured by parameter-sign region.

In a numerical companion section, the family is screened against a
30-index classical baseline on three QSPR benchmarks (ESOL,
FreeSolv, Lipophilicity; total $5\,966$ molecules) using a
transparent orthogonality protocol. Every grid point on ESOL and
Lipophilicity is highly correlated with at least one classical
baseline ($|r|\ge 0.95$); one of the $100$ grid points on FreeSolv,
at $(\alpha,\beta,\gamma)=(-1,-1,1)$, falls just below the pairwise
threshold with $\max|r|=0.943$, but at that grid point the Wuzi
value is exactly captured by a linear combination of the $30$
baseline indices and its partial correlation with the target is
zero. The empirical behaviour is consistent
with a structural observation: every endpoint-degree edge-sum index
on hydrogen-suppressed molecular graphs with maximum degree
$\Delta\le 4$ lies in a vector space of dimension at most~$10$,
spanned by the ten edge-degree-pair counts.
...
chemometric program (Section~\ref{sec:numerical}) uses an
...
\item Sharp bounds in $(n,m,\Delta,\delta)$ and in classical
indices, stated as theorems with explicit proof strategies and
partial proofs (Sections~\ref{sec:bounds_nm}--\ref{sec:bounds_classical}).
...
sensitivity on decane isomers (Section~\ref{sec:numerical}).
\end{enumerate}
...
(Table~\ref{tab:specialcases}). The novel feature is the
multiplicative $\exp(\gamma\cdot \mathrm{imb}(d_u,d_v))$ factor,
where $\mathrm{imb}(x,y)=|x-y|/(x+y)\in[0,1)$ is the normalized
degree imbalance. The factor is bounded above by $e^{\gamma}$ and
reduces to $1$ on regular graphs.
...
Both inequalities become equalities if and only if every edge of
$G$ shares the same degree pair, which holds in particular for
...
summed over the $m$ edges. Equality in~\eqref{eq:bracket} requires
that every term in the sum equals the bound; since $\psi$ is
strictly determined by the unordered pair $(d_u,d_v)$, this means
every edge has the same unordered degree pair.
\end{proof}
...
redundancy that emerges in Section~\ref{sec:numerical}.
...
redundancy screening in Section~\ref{sec:numerical}.
\end{remark}
...
\subsection{Bounds involving $M_1$ and $m$ (proof skeletons)}
...
Equality holds iff $d_u+d_v$ is constant on every edge of~$G$.
\end{theorem}
...
Multiplying both sides by $m$ gives the stated bound. Equality
follows from the equality condition of Jensen.
\end{proof}

\begin{theorem}[Bound in $M_1$ and $m$, $0<\beta<1$ -- skeleton]
\label{thm:m1_concave}
Let $\alpha=\gamma=0$ and $0<\beta<1$. Then
\[
\Wuzi=\sum_{e}(d_u+d_v)^{\beta}\;\le\; m^{1-\beta}M_1(G)^{\beta}.
\]
\todo{PROOF SKETCH -- TO BE COMPLETED} Concavity of $t\mapsto t^\beta$
on $\mathbb{R}_{>0}$ for $\beta\in(0,1)$ and Jensen
(Lemma~\ref{lem:jensen}) give the reversed inequality. The
equality condition and any sharper version with explicit
dependence on $\Delta$ and $\delta$ are left for the formal
derivation.
\end{theorem}
...
A clean and verifiable conclusion of this section is the
following.

\begin{proposition}\label{prop:tightness_regular}
Among all $r$-regular graphs of order $n$ and size
$m=\tfrac{nr}{2}$, the value $\Wuzi=m\cdot 2^{\beta} r^{2\alpha+\beta}$
is invariant under graph automorphism. In particular, the bracket
of Theorem~\ref{thm:bracket} is tight on $K_n$, $C_n$, $Q_k$, and
any other $r$-regular graph.
\end{proposition}

\begin{proof}
Direct from Theorem~\ref{thm:regular_bracket}.
\end{proof}
...
\begin{table}[h]
\centering
\caption{Summary of extremal-graph conjectures for $\Wuzi$. ``$+$'' denotes the sign region in which the conjecture is stated.}
\label{tab:extremals}
\begin{tabular}{lcccc}
\toprule
Graph class & $\alpha$ & $\beta$ & $\gamma$ & Conjectured extremals (max / min)\\
\midrule
Trees, order $n\ge 3$ & $+$ & $+$ & $+$ & $S_n$ / $P_n$\\
Trees, order $n\ge 3$ & $-$ & $-$ & $0$ & $P_n$ / $S_n$ (mirror)\\
Unicyclic, $n\ge 4$ & $+$ & $+$ & $+$ & $U_n^{(1)}$ / $C_n$\\
Bicyclic, $n\ge 5$ & $+$ & $+$ & $+$ & \todo{TO BE CHARACTERIZED}\\
\bottomrule
\end{tabular}
\end{table}

% =====================================================================
\section{Numerical work}\label{sec:numerical}
% =====================================================================

This section reports the numerical work on the Wuzi family. All
computations are reproducible from the public repository.

\subsection{Prediction on octanes}\label{sec:octanes}

Following the convention of Section~5.1
of~\cite{MovahediGutmanRedzepovicFurtula2026}, we measure the
Pearson correlation of the Wuzi family at a panel of canonical
parameter points with eight physicochemical properties of the
$n=18$ octane isomers (boiling point, melting point, enthalpy of
formation, enthalpy of vaporization, entropy, accentric factor,
density, refractive index; data from NIST WebBook).

Figure~\ref{fig:octanes} shows the signed Pearson $r$ between each
of $27$ indices (Wuzi at canonical $(\alpha,\beta,\gamma)$
plus the $30$-index classical baseline, less three indices
constant on the $18$ octanes) and each of the eight properties.
Padmakar--Ivan ($PI$) is degenerate on the $18$ octanes
($\sigma=0$, mean $42$) and is excluded. Strong performance on this
small dataset is necessary but not sufficient for general QSPR
utility (cf.\ Section~\ref{sec:cross_dataset}).
...
\subsection{Cross-dataset orthogonality screening}\label{sec:cross_dataset}

We screen the Wuzi family on a $100$-point parameter grid
$(\alpha,\beta,\gamma)\in\{-1,-0.5,0,0.5,1\}^2\times\{0,0.5,1,2\}$
against a $30$-index classical baseline on three MoleculeNet QSPR
benchmark datasets~\cite{MoleculeNet}:
\begin{itemize}
\item \textbf{ESOL} \cite{Delaney2004ESOL}: $n=1\,127$ molecules,
target $\log S$ (aqueous solubility, mol/L).
\item \textbf{FreeSolv} \cite{Mobley2014FreeSolv}: $n=639$ molecules,
target $\Delta G_{\mathrm{hyd}}$ (hydration free energy, kcal/mol).
\item \textbf{Lipophilicity} \cite{Wang2009ChEMBL}: $n=4\,200$
molecules, target $\log D$ at pH~7.4.
\end{itemize}

Two diagnostics are computed for each grid point and dataset:
$\max_b |r(W_e, X_b)|$ over the $30$ baseline indices $X_b$, and
$|r(W_e^{\mathrm{res}}, y^{\mathrm{res}})|$ where $W_e^{\mathrm{res}}$
and $y^{\mathrm{res}}$ are the residuals after regressing the
Wuzi value $W_e$ and target $y$ on the $30$ baselines (partial
correlation with target).

\begin{table}[h]
\centering
\caption{Baseline redundancy and Wuzi-family screening verdict on three QSPR benchmarks. Pair counts are out of $\binom{30}{2}=435$. PC$_{95}$ is the smallest $k$ with cumulative explained variance $\ge 0.95$.}
\label{tab:cross_dataset}
\begin{tabular}{lrrrrrr}
\toprule
Dataset & $n$ & $|r|\ge 0.90$ & $|r|\ge 0.95$ & PC$_{95}$
        & Wuzi pass / $100$
        & Min max $|r|$ \\
\midrule
ESOL          & 1127 & 203 (46.7\%) & 159 (36.6\%) & 3 & 0 & 0.965\\
FreeSolv      &  639 & 257 (59.1\%) & 162 (37.2\%) & 3 & 1 & 0.943\\
Lipophilicity & 4200 & 200 (46.0\%) & 164 (37.7\%) & 3 & 0 & 0.965\\
\bottomrule
\end{tabular}
\end{table}

The empirical pattern (Table~\ref{tab:cross_dataset},
Figs.~\ref{fig:pca}--\ref{fig:wuzi_grid_all}) is the following:

\begin{enumerate}[label=(\arabic*)]
\item Roughly $37\%$ of the $\binom{30}{2}=435$ pairwise correlations
between classical indices exceed the $|r|=0.95$ threshold on
every dataset.
\item Three principal components capture $95\%$ of the variance of
the standardized $30$-index baseline on every dataset; five
components capture $99\%$.
\item On ESOL and Lipophilicity, every one of the $100$ Wuzi grid
points has $\max|r|$ with some classical baseline above $0.96$.
On FreeSolv, a single grid point at $(\alpha,\beta,\gamma)=(-1,-1,1)$
has $\max|r|=0.943$, marginally below the pairwise threshold;
however, at that grid point the Wuzi value is captured by an exact
linear combination of the $30$ baseline indices (the residual
after regression has variance below numerical precision), so the
partial correlation with $\Delta G_{\mathrm{hyd}}$ is identically
zero. This illustrates that pairwise correlation alone catches only
one-dimensional redundancy; the multi-dimensional regression
residual is the load-bearing diagnostic.
\item The maximum $|$partial $r$ with target$|$ across all $300$
grid points is $0.036$ (FreeSolv) and $0.0093$ (Lipophilicity);
both maxima occur at non-passing grid points and both are below
the $0.10$ threshold conventionally used to assess meaningful added
signal.
\end{enumerate}

This behaviour is consistent with Corollary~\ref{cor:bid_redundancy}:
the Wuzi family is parametrized in $\mathbb{R}^3$ but lives in a
vector space of dimension at most~$10$ on $\mathcal{G}_4$.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{fig2_pca_scree.pdf}
\caption{Cumulative explained variance of PCA on the $30$-index
baseline for the three datasets. Three components capture $95\%$
of the variance on every dataset.}
\label{fig:pca}
\end{figure}
...
\caption{$\max|r|$ between the Wuzi index at each grid point
$(\alpha,\beta,\gamma)$ and any of the 30 classical baseline
indices, across three datasets (rows) and four $\gamma$ slices
(columns). The single starred cell (FreeSolv, $(-1,-1,1)$) falls
below the threshold with $\max|r|=0.943$; its partial correlation
with the target is approximately zero.}
...
recovers $M_2,R,\chi,H$ as special cases, and admits sharp
bracket bounds via the edge-contribution function. The bracket
becomes an exact identity on regular graphs.

At the same time the chemometric behaviour
(Section~\ref{sec:numerical}) is consistent across three
independent QSPR benchmarks: the family lies within the $10$-dimensional
BID subspace on $\mathcal{G}_4$, and on the $300$ grid points sampled
only one (FreeSolv $(-1,-1,1)$) is below the conventional $|r|=0.95$
redundancy threshold against the $30$-index classical baseline.
The borderline FreeSolv grid point carries essentially zero partial
correlation with $\Delta G_{\mathrm{hyd}}$, so passing the
correlation screen does not translate into independent QSPR signal.
...
the mathematical side (bounds and extremal graphs in the
classical tradition); on the chemometric side it offers no escape
from the BID redundancy ceiling but it provides a transparent
worked example of how to screen a parametric extension against an
existing zoo of descriptors. We do not claim the family is
``superior'' to any classical index, and the BID-basis observation
of Theorem~\ref{thm:bid} explicitly accounts for the redundancy
seen in Section~\ref{sec:numerical}.
...
On the chemometric side, future work includes:
\begin{enumerate}
\item Extension of the screening pipeline to non-BID candidate
indices (information-theoretic, centrality, motif, eccentricity
families), reported in the companion methodology paper.
\item Sensitivity of the conclusions to the redundancy threshold:
the conventional choice $|r|=0.95$ is reported here, with the
empirical pattern stable across $|r|\in[0.90, 0.99]$.
\end{enumerate}
...
bracket bounds via the edge-contribution function, with the bracket
being tight on regular graphs, (iii) given exact reductions to
classical indices $M_2,R,\chi,H$ at six parameter points, and
(iv) reported numerical evidence on three QSPR benchmarks showing
that the family is highly correlated with at least one classical
baseline index at every tested grid point except one borderline
case which does not carry independent QSPR signal. The latter
behaviour is structurally explained by the Edge-Degree-Pair Basis
observation, which gives a $10$-dimensional ceiling on the BID
family on $\mathcal{G}_4$.
...
\todo{TO FILL} Funding sources, institutional support, and
acknowledgements to colleagues who provided feedback on the
manuscript.
...
\emph{FreeSolv: A database of experimental and calculated hydration
...
A.~S.~Pappu, K.~Leswing, and V.~Pande,
