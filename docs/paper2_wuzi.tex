...
attained for $n \ge 6$ by a non-classical caterpillar tree whose
formal characterization is left as an open problem.
...
baseline ($|r|\ge 0.95$); only the FreeSolv grid point at
$(\alpha,\beta,\gamma)=(-1,-1,1)$ falls just below the pairwise
threshold ($\max|r|=0.943$). At this parameter triple the Wuzi
value is exactly captured by a linear combination of the $30$
baseline indices on \emph{all three} datasets, so the partial
correlation with the respective target is identically zero in each
case. The empirical behaviour is consistent with a structural
observation: every \emph{bond-incident-degree} index --- a
real-valued functional of the form
$I_f(G) = \sum_{uv \in E(G)} f(d_u, d_v)$ for some symmetric
$f$ --- on hydrogen-suppressed molecular graphs with maximum
degree $\Delta\le 4$ lies in a vector space of dimension at
most~$10$, spanned by the ten edge-degree-pair counts.
...
\subsection{Computational evidence and a non-classical caterpillar regime}
...
is \emph{neither} $P_n$ nor $S_n$ for $n \ge 6$. The argmax is a
caterpillar tree with one or two branch vertices; representative
degree sequences are listed in Table~\ref{tab:caterpillar_argmax}.
\end{enumerate}

\begin{table}[h]
\centering
\caption{Observed maximizer of $W(T; -1, -1, 1)$ over all non-isomorphic trees of order $n$, identified by degree sequence (descending). For $n = 5$ the maximum is attained at the path $P_5$; for $n \ge 6$ the maximum is a non-classical caterpillar tree.}
...
\begin{conjecture}[Mixed-sign extremal trees]
...
order $n$ is attained by a caterpillar tree: a tree whose internal
vertices form a path, with pendant vertices attached only along
that path.
\end{conjecture}

The conjecture, if true, is the genuinely new mathematical
phenomenon of this parametric family: a parameter triple in which
the standard star / path extremals fail and a caterpillar regime
takes over. A formal extremal-graph characterization in this
regime --- which caterpillar attains the maximum, and what are the
equality conditions --- is left for the formal derivation.
...
Trees, $n\ge 6$ & $\circ$ & $\circ$ & $\circ$ & caterpillar / $S_n$\\
...
($\sigma=0$, mean $42$) and is excluded. The degeneracy is
structural: on any tree of order $n$ and size $m = n - 1$,
$PI(T) = m(m - 1)$ because for every edge $e$ removal disconnects
the tree into two parts and every other edge lies entirely in one
of those two parts; the $18$ octanes are all trees on $8$ vertices,
giving $PI \equiv 7 \cdot 6 = 42$. Strong performance on this
...
\item \textbf{Lipophilicity} \cite{LipophilicityChEMBL}: $n=4\,200$
...
A separate observation, dataset-independent, applies to this same
parameter triple: at $(\alpha,\beta,\gamma)=(-1,-1,1)$ the Wuzi
value is captured exactly by a linear combination of the $30$
baseline indices on \emph{all three} datasets (the residual after
regression has variance below numerical precision on ESOL, FreeSolv,
and Lipophilicity alike), so the partial correlation with the
respective target is identically zero in each case. The pairwise
$\max|r|$ at this point varies by dataset ($0.965$, $0.943$, $0.977$
on ESOL / FreeSolv / Lipo respectively, all with $mM_2$), but the
multi-dimensional redundancy is structural. This illustrates that
pairwise correlation alone catches only one-dimensional redundancy;
the regression residual is the load-bearing diagnostic.
