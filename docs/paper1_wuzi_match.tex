...
\begin{abstract}
For a simple connected graph $G = (V, E)$ with edge degree
pairs $(d_u, d_v)$, the \emph{Wuzi index family} is the
three-parameter bond-incident-degree (BID) index
$W(G; \alpha, \beta, \gamma) =
\sum_{uv \in E(G)} (d_u d_v)^{\alpha} (d_u + d_v)^{\beta}
\exp(\gamma\,|d_u - d_v|/(d_u + d_v))$
with real parameters $\alpha, \beta, \gamma$. The family
recovers many classical degree-based indices --- the first and
second Zagreb indices $M_1, M_2$, the hyper-Zagreb index $HM$,
the modified second Zagreb index ${}^{m}\!M_2$, the
Randi\'c index $R$, the general Randi\'c $R_\alpha$, the
sum-connectivity index $\chi$, the general sum-connectivity
$\chi_\beta$, the half-harmonic index $H/2$, and the inverse-sum
indeg index $ISI$ exactly, and the geometric-arithmetic index
$GA$ and arithmetic-geometric index $AG$ up to scale --- as
exact reductions at specific parameter triples. We make three
contributions. First, we develop a coherent bounds programme for
$\Wuzi$: closed forms on nine standard graph classes, Jensen-type
bounds via $M_1$ and $M_2$, Cauchy--Schwarz parameter-splitting
bounds, bounds in $(n, m, \Delta, \delta)$, a uniform ratio
bound, and bounds via the generalised Randi\'c,
sum-connectivity, and Sombor indices. Second, exhaustive
enumeration through $n = 20$ at the mixed-sign triple
$(\alpha, \beta, \gamma) = (-1, -1, 1)$, together with
sign-region checks through $n = 12$ at seven additional
canonical triples, isolates a non-classical extremal regime:
the maximizer over non-isomorphic trees of order $n$ is neither
a path, a star, nor a caterpillar for $n \ge 7$, with a sharp
odd/even pattern (spiders $S^{(2)}_{(n-1)/2}$ for odd $n \ge 9$;
balanced double-spiders $DS(a, b)$ with $a + b = n/2 + 1$ for
even $n \ge 10$), stated formally as
Conjecture~\ref{conj:caterpillar_max}. Third, a sensitivity
benchmark on the $18$ octane isomers places the Wuzi family in
the same correlation regime as the classical BID indices, with
all margins within the bootstrap-CI half-widths characteristic
of $n = 18$; no claim of QSPR superiority is made.
\end{abstract}
...
The main motivation of the present paper is the
\emph{mixed-sign extremal behaviour} of a parametric BID family
rather than a claim of universal QSPR superiority. We introduce
the \emph{Wuzi index family} as a controlled three-parameter BID
setting in which the standard path/star extremal pattern (on
trees, in the non-negative parameter region) can be compared
directly with a non-classical regime that emerges at the
mixed-sign parameter triple
$(\alpha, \beta, \gamma) = (-1, -1, 1)$. Exhaustive enumeration
of all non-isomorphic trees of order $5 \le n \le 20$ at this
triple produces a stable spider / double-spider maximizer
pattern that is neither caterpillar nor classical for $n \ge 7$
(Section~\ref{sec:extremal}, Table~\ref{tab:caterpillar_argmax},
Figure~\ref{fig:caterpillar_maximizers}), stated formally as
Conjecture~\ref{conj:caterpillar_max}. The remaining bounds and
reductions of Section~\ref{sec:bounds} locate this behaviour
inside the standard BID framework, with the Wuzi family
recovering $M_2, R, \chi, H$ as boundary cases.

A recurring methodological question for any newly proposed index
is: what does it have to demonstrate to be a meaningful
contribution? On the mathematical side, closed-form values on
standard graph classes, sharp upper and lower bounds in terms of
graph parameters and other classical indices, and an
extremal-graph characterization (among trees, unicyclic, and
bicyclic graphs) form the established checklist. On the
chemometric side, the index must add information not already
carried by the existing zoo of descriptors. The Wuzi family's
contribution against this checklist is positioned narrowly: we
provide the closed forms, bounds, and reductions in the standard
form expected by recent parametric-BID papers such
as~\cite{MovahediGutmanRedzepovicFurtula2026}, and the empirical
sensitivity benchmark on octanes shows the family in the same
correlation regime as the classical BID indices rather than
above it. The distinctive content is therefore the mixed-sign
extremal phenomenon and its odd/even structural pattern, not a
claim of QSPR superiority.

Direct substitution into Equation~\eqref{eq:wuzi} shows that the
Wuzi family contains a wide range of classical BID indices as
exact reductions at specific parameter triples, either
identically or up to a multiplicative scaling constant (by the
same convention used here for $H/2$, recovered at
$(0, -1, 0)$). The reductions verified by direct substitution
include the edge count $m$ at $(0, 0, 0)$, the first Zagreb
index $M_1$~\cite{Gutman1972} at $(0, 1, 0)$, the second Zagreb
index $M_2$~\cite{Gutman1972} at $(1, 0, 0)$, the hyper-Zagreb
index $HM$ at $(0, 2, 0)$, the modified second Zagreb index
${}^{m}\!M_2$~\cite{NikolicKovacevicMilicevicTrinajstic2003} at
$(-1, 0, 0)$, the Randi\'c index $R$~\cite{Randic1975} at
$(-1/2, 0, 0)$, the sum-connectivity index
$\chi$~\cite{ZhouTrinajstic2009sumconn} at $(0, -1/2, 0)$, the
half harmonic index $H/2$~\cite{Fajtlowicz1987Harmonic} at
$(0, -1, 0)$, the inverse sum indeg index
$ISI$~\cite{Vukicevic2010BondAdditive2} at $(1, -1, 0)$, the
geometric-arithmetic index
$GA$~\cite{VukicevicFurtula2009GA} (up to a factor of $2$) at
...
$\beta = \gamma = 0$ axis $W(G; \alpha, 0, 0)$, and the general
sum-connectivity family
$\chi_\beta$~\cite{ZhouTrinajstic2009sumconn} as the
$\alpha = \gamma = 0$ axis $W(G; 0, \beta, 0)$. The Wuzi family
thus interpolates among the major degree-based BID indices,
parametrically connecting them through a single three-parameter
object. Indices whose summand cannot be written as
$(d_u d_v)^\alpha (d_u + d_v)^\beta$ up to a constant scaling
are \emph{not} Wuzi reductions; these include the Sombor index
$SO$~\cite{Gutman2021Sombor}, the augmented Zagreb index $AZI$,
the atom-bond-connectivity index $ABC$, the forgotten index
$F$~\cite{FurtulaGutman2015F}, the symmetric division degree
index $SDD$, and the Albertson irregularity
index~\cite{Albertson1997}.

The distinguishing feature of the Wuzi family within the
...
As an instance of the bond-incident-degree (BID) class on
hydrogen-suppressed molecular graphs of maximum degree
$\Delta \le 4$, the entire Wuzi family
$\{\Wuzi : (\alpha, \beta, \gamma) \in \mathbb{R}^3\}$ lies in a
real vector space of dimension at most ten --- the standard span
of the edge-degree-pair counts $m_{ij}(G) = |\{uv \in E(G) :
\{d_u, d_v\} = \{i, j\}\}|$ for $1 \le i \le j \le 4$, as
documented in~\cite{BorovicaninDasFurtulaGutman2017Zagreb}.
A companion methodology paper~\cite{Paper2_Screening} uses this
ten-dimensional ceiling as the structural reason that no
parametric BID family on this molecular class can escape
redundancy with a sufficiently large classical baseline; the
present paper is the mathematical and chemometric study of the
Wuzi family itself.
...
reductions catalogued in Section~\ref{sec:intro} the constants
...
\centering
\small
\caption{Observed maximizer of $W(T; -1, -1, 1)$ over all non-isomorphic trees of order $5 \le n \le 20$.}
...
\vspace{0.4em}
\noindent\footnotesize{Maximizers identified by degree sequence (descending) and canonical \mbox{\texttt{graph6}} string. For $n = 5$ the maximizer is the path $P_5$; for $n = 6$ a caterpillar; for $n \in \{7, \ldots, 20\}$ neither a path, a star, nor a caterpillar. Odd $n \in \{7, 9, 11, 13, 15, 17, 19\}$: spider with hub degree $h = (n-1)/2$ and length-$2$ spokes (degree sequence $(h, 2^h, 1^h)$). Even $n \in \{10, 12, 14, 16, 18, 20\}$: balanced double-spider with two adjacent hubs of degrees $a, b$, $a + b = n/2 + 1$, $|a - b| \le 1$. The case $n = 8$ is a spider with one spoke extended to length $3$. Computed by exhaustive enumeration of all non-isomorphic trees of order $n$ ($3$ at $n = 5$, $823\,065$ at $n = 20$; OEIS sequence A000055).}
\end{table}
...
\begin{table}[H]
\centering
\caption{Pearson correlation $r$ between each index and each of the five physicochemical properties of the $18$ octane isomers. Bold entries flag the per-column maximum $|r|$.}
...
\vspace{0.4em}
\noindent\footnotesize{\textbf{Bootstrap uncertainty} (B = $10\,000$, percentile method, $95\%$ CI): headline values are $r(\Delta H_{\mathrm{vap}}, GA) = +0.959$ with CI $[+0.917, +0.986]$, $r(\Delta H_{\mathrm{vap}}, H) = +0.959$ with CI $[+0.917, +0.986]$, and $r(\Delta H_{\mathrm{vap}}, W(0,0,2)) = -0.967$ with CI $[-0.991, -0.921]$. The CI half-widths range from $0.019$ to $0.47$, median $0.13$; differences in $|r|$ smaller than ${\sim}0.10$--$0.15$ are within bootstrap noise at $n = 18$. \textbf{Leave-one-out cross-validation} across the $18$ isomers gives maximum drift in $|r|$ of at most $0.011$ on the headline $\Delta H_{\mathrm{vap}}$ cells, attributable to $2,3,3$-trimethylpentane, confirming that the headline correlations are not driven by a single outlying isomer.}
\end{table}
...
in agreement with the exact reductions catalogued in
Section~\ref{sec:intro}; these rows function as an internal
consistency check.
...
$W(1, 0, 0) = M_2$ with $|r| = 0.987$, which is the second
Zagreb index itself by the exact reduction at $(1, 0, 0)$
...
The Wuzi family is a three-parameter parametric BID family that
recovers a wide range of classical degree-based indices as exact
reductions and sits inside the same finite-dimensional BID
descriptor subspace as the established classical indices.
Mathematically, we have established a coherent programme of
bounds on $\Wuzi$ --- a bracket bound that is tight on regular
graphs, Cauchy--Schwarz parameter-splitting bounds, Jensen-type
bounds via the first and second Zagreb indices,
$(n, m, \Delta, \delta)$ bounds in the non-negative parameter
region, a uniform ratio bound that sandwiches $\Wuzi$ between
explicit multiples of any classical BID index with strictly
positive edge contribution, and bounds via the generalised
Randi\'c, sum-connectivity, and Sombor indices.

Empirically, the principal new phenomenon of the paper is the
non-classical extremal regime at
$(\alpha, \beta, \gamma) = (-1, -1, 1)$. Let $S^{(2)}_{h}$ denote
the spider with hub degree $h$ and length-$2$ spokes (the tree
on $2h + 1$ vertices consisting of a single hub of degree $h$
together with $h$ length-$2$ paths), and let $DS(a, b)$ denote
the balanced double-spider with hub degrees $a, b$ (the tree on
$2(a + b - 1)$ vertices obtained by joining two hubs of degrees
$a, b$ by an edge and attaching $a - 1$ and $b - 1$ length-$2$
paths). Exhaustive enumeration of all non-isomorphic trees of
order $5 \le n \le 20$ supports the conjecture:
...
Chemometrically, a sensitivity benchmark on the $18$ octanes
places the Wuzi family in the same correlation regime as the
classical BID indices, with all margins within the bootstrap-CI
half-widths characteristic of $n = 18$; the Wuzi family is
therefore competitive with --- not superior to --- the classical
indices on this benchmark, and we make no claim of QSPR
superiority. The Wuzi family is best viewed as a controlled
parametric probe of the finite BID descriptor space, with the
mixed-sign spider/double-spider regime at $(-1, -1, 1)$ as the
principal new extremal phenomenon.

The open problems suggested by the present work are the
following.
\begin{enumerate}
\item A formal proof of Conjecture~\ref{conj:caterpillar_max}
along the classical transformation-lemma template: show that any
tree $T$ of order $n \ge 9$ not isomorphic to the prediction
admits a local edge transformation strictly increasing
$W(T; -1, -1, 1)$. This is the principal mathematical open
question of the paper.
\item Sharp Nordhaus--Gaddum sum bounds for $\Wuzi$ in the spirit
of the classical Zagreb-index
results~\cite{NordhausGaddumOrig1956}.
\item Sharp two-sided bounds for $\Wuzi$ via the
geometric--arithmetic index
$GA$~\cite{VukicevicFurtula2009GA}, the harmonic index $H$, and
the atom-bond-connectivity index $ABC$.
\item Formal extremal-graph characterisations for unicyclic and
bicyclic graphs in the spirit of~\cite{LiuLuTian2008Randic,
HansenMelot2003unicyclicRandic}.
\item A formal study of the Albertson~\cite{Albertson1997}
irregularity pairing
$\mathrm{Alb}(G) = \sum_{uv \in E(G)} |d_u - d_v|$ as the natural
mathematical companion of the $\gamma$-axis of $\Wuzi$.
\item Chemometric extension to additional compound classes
