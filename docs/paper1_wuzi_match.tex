...
computationally. Exhaustive enumeration of all non-isomorphic
trees of order $5 \le n \le 20$ at eight canonical parameter
triples partitions parameter space into three sign regions, two
of which reproduce classical extremal behaviour and one of which
exhibits a non-classical mixed-sign regime at
$(\alpha, \beta, \gamma) = (-1, -1, 1)$. The mixed-sign triple is
enumerated through $n = 20$ (Table~\ref{tab:caterpillar_argmax});
the other seven triples are sign-region checks enumerated through
$n = 12$, where the sign-region pattern is already empirically
stable. The three sign regions of the parameter panel are as
follows.
