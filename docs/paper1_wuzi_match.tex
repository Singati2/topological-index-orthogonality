...
\begin{abstract}
Degree-based topological indices assign to a molecule a single
number computed from the degrees of its adjacent atoms; dozens
have been proposed since the 1970s, including the Zagreb,
Randi\'c, sum-connectivity, harmonic, and Sombor indices. We
introduce the \emph{Wuzi index family}, a three-parameter
degree-based index (defined in Section~\ref{sec:def}) built from
the product, the sum, and the normalized difference of each pair
of adjacent degrees. At specific values of its three parameters
the family reduces exactly to many of these classical indices
--- among them the first and second Zagreb, Randi\'c,
sum-connectivity, harmonic, and inverse-sum-indeg indices. We
derive closed-form values on standard graph classes and a family
of sharp upper and lower bounds. Exhaustive enumeration of all
trees of order up to twenty reveals a non-classical extremal
pattern at a mixed-sign parameter choice: for order at least
seven the maximizing tree is neither a path, a star, nor a
caterpillar, but a spider or a balanced double-spider following a
sharp odd/even rule. A sensitivity benchmark on octane, tree, and
decane test sets places the family in the same correlation regime
as the classical indices; no claim of predictive superiority is
made.
\end{abstract}
...
the rapidly growing family has recently been surveyed
comprehensively by Gutman~\cite{Gutman2026SomborSurvey}.
...
Conjecture~\ref{conj:caterpillar_max}. The bounds of
Section~\ref{sec:bounds} locate this behaviour inside the
standard BID framework, while the exact reductions catalogued at
the end of this section show the Wuzi family recovering many
classical indices --- including the first and second Zagreb,
Randi\'c, sum-connectivity, harmonic, and inverse-sum-indeg
indices --- as special cases.

A recurring methodological question for any newly proposed index
is what it must demonstrate to be a meaningful contribution
rather than a redundant reparametrization of known descriptors.
On the mathematical side, the established checklist comprises
...
classical indices, and an extremal-graph characterization among
trees and, where possible, unicyclic and bicyclic graphs. On the
chemometric side, a new index must carry predictive information
not already supplied by the existing collection of descriptors.
We position the Wuzi family against this checklist deliberately
narrowly. We supply the closed forms, the bounds, and the exact
reductions in the form expected by recent parametric-BID
papers~\cite{MovahediGutmanRedzepovicFurtula2026}, and the
empirical sensitivity benchmark on the octane isomers places the
family in the same correlation regime as the classical BID
indices rather than above them. The distinctive contribution is
therefore not QSPR superiority but the mixed-sign extremal
phenomenon and its sharp odd/even structural pattern, which is
...
It is useful to record, finally, the descriptor-space ceiling in
...
the Wuzi family, like every BID family on this molecular class,
lies inside a fixed ten-dimensional descriptor
space~\cite{BorovicaninDasFurtulaGutman2017Zagreb}, no matter how
its three real parameters are chosen. The redundancy consequences
of this ceiling --- in particular the extent to which a
parametric BID family can or cannot escape collinearity with a
sufficiently large classical baseline --- are taken up in a
...
\caption{Maximizers of $W(T; -1, -1, 1)$ over all non-isomorphic trees of order $n$, for $n = 10, 11, 12$. Internal vertices are coloured green and leaves cream. None of the three trees is a caterpillar (the non-leaf vertices do not form a path). $n = 10$ is the balanced double-spider $DS(3, 3)$; $n = 11$ is the spider $S^{(2)}_{5}$ with $h = (11 - 1)/2 = 5$ length-$2$ spokes; and $n = 12$ is $DS(4, 3)$.}
...
The Wuzi family is a three-parameter degree-based index that
...
graph classes and a programme of sharp upper and lower bounds: a
bracket bound tight on regular graphs, Cauchy--Schwarz and
Jensen-type bounds, bounds in the basic graph parameters
$n, m, \Delta, \delta$, a uniform ratio bound against any
classical index with positive edge contribution, and bounds via
the generalised Randi\'c, sum-connectivity, and Sombor indices.

The principal new phenomenon is a non-classical extremal regime
at the mixed-sign triple $(\alpha, \beta, \gamma) = (-1, -1, 1)$.
Exhaustive enumeration of all non-isomorphic trees of order
$5 \le n \le 20$ shows that, for $n \ge 7$, the maximizing tree
is neither a path, a star, nor a caterpillar, but a spider for
odd $n$ and a balanced double-spider for even $n$. Writing
$S^{(2)}_{h}$ for the spider with one hub of degree $h$ and $h$
length-$2$ spokes, and $DS(a, b)$ for the double-spider formed by
joining two hubs of degrees $a, b$ and attaching length-$2$
spokes, this is recorded as the following conjecture:
...
\bibitem{Gutman2026SomborSurvey} I. Gutman, Survey of
    Sombor-type degree-based topological indices,
    \textit{MATCH Commun. Math. Comput. Chem.\/}
    \textbf{96} (2026) 819--837.
...
\bibitem{Rada2026LinearVDB} J. Rada, A finite-dimensional linear
    framework for vertex-degree-based topological indices,
    \textit{MATCH Commun. Math. Comput. Chem.\/}
    \textbf{96} (2026) 841--863.
