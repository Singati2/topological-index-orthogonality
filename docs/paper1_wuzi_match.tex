...
\begin{abstract}
We isolate a non-classical extremal regime for a parametric
bond-incident-degree (BID) index family on trees. At the
mixed-sign parameter triple
$(\alpha, \beta, \gamma) = (-1, -1, 1)$, exhaustive enumeration
through $n = 20$ shows that the maximizer over non-isomorphic
trees is neither the path nor the star, and is not a caterpillar
for every $n \in \{7, \ldots, 20\}$, with a sharp odd/even
structural pattern: spiders $S^{(2)}_{(n-1)/2}$ with length-$2$
spokes for odd $n \ge 9$, and balanced double-spiders
$DS(a, b)$ with $a + b = n/2 + 1$ and $|a - b| \le 1$ for even
$n \ge 10$. This computational evidence is stated as
Conjecture~\ref{conj:caterpillar_max}; a formal
transformation-lemma proof is left open.

The parametric setting in which this regime emerges is the
\emph{Wuzi index family}: for a simple connected graph
$G = (V, E)$ with edge degree pairs $(d_u, d_v)$,
\[
\Wuzi
\;=\; \sum_{uv \in E(G)}
...
through the third parameter~$\gamma$. We derive closed-form
values on nine standard graph classes, a bracket bound via the
...
$(n, m, \Delta, \delta)$ for the non-negative parameter region,
and a uniform ratio-bound theorem that sandwiches $\Wuzi$ between
explicit multiples of any classical BID index with strictly
positive edge contribution. The exhaustive tree enumeration
covers eight canonical parameter triples: the mixed-sign triple
$(-1, -1, 1)$ is enumerated through $n = 20$, while the
remaining seven triples serve as sign-region checks through
$n = 12$. A sensitivity analysis on the standard $18$-octane
benchmark places the family in the same correlation regime as
the classical BID indices, with all margins below the
bootstrap-CI half-widths characteristic of $n = 18$; we make no
claim of QSPR superiority over established indices.
\end{abstract}
