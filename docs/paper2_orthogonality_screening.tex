...
(\S\ref{sub:threshold_sensitivity}). An ESOL single-dataset case
study on $20$ alternative-family candidates (information-theoretic,
centrality, motif, eccentricity, spectral-hybrid) shows that
$15/20$ clear the pairwise threshold but only $4/20$ also clear
$\lvert\mathrm{pcor}(z, y \mid X)\rvert \ge 0.10$: pairwise-only
screening is over-permissive on this dataset, but cross-dataset
generality of the alternative-family result is not claimed. A
5-fold cross-validated RandomForest benchmark on those three
regression datasets plus BBBP ($n = 2\,039$) shows the
pairwise-pruned feature set preserves predictive performance within
CV noise at roughly $4\times$ fewer features
(Table~\ref{tab:ml_benchmark}), and is competitive with LASSO and
ElasticNet linear comparators (Table~\ref{tab:fs_comparison}).
...
and linear diagnostics, or chemical utility of the four ESOL
alternative-family passers; full limitations are catalogued in
...
\centering
\small
\caption{Wuzi $100$-point grid: pass counts under threshold variation. Top block: pairwise screen only ($\max_b\lvert r(W, X_b)\rvert < \tau_r$). Bottom block: combined diagnostic (pairwise pass \emph{and} $\lvert\mathrm{pcor}(W, y \mid X)\rvert \ge \tau_{\mathrm{pcor}}$).}
...
\begin{tabular}{lccc}
\toprule
& \multicolumn{3}{c}{Combined-diagnostic pass count, $\tau_r = 0.95$}\\
\cmidrule(lr){2-4}
Dataset & $\tau_{\mathrm{pcor}} = 0.05$ & $\tau_{\mathrm{pcor}} = 0.10$ & $\tau_{\mathrm{pcor}} = 0.20$\\
\midrule
ESOL          & $0$ & $0$ & $0$\\
FreeSolv      & $0$ & $0$ & $0$\\
Lipophilicity & $0$ & $0$ & $0$\\
...
\noindent\emph{Scope note.} This case study is presented as an
\emph{ESOL-only single-dataset case study}, not as a cross-dataset
generalisation. The Wuzi case study above replicates across the
three regression datasets because the Wuzi candidate indices are
parametric BID functions that are cheap to evaluate on already-cached
descriptor matrices; the twenty alternative-family candidates here
require building hydrogen-suppressed molecular graphs and running
heavier per-graph computations (betweenness, eccentricity, motif
counts, spectral hybrids), and cross-dataset replication of this
specific 20-candidate screen on FreeSolv, Lipophilicity, and BBBP
is left as future work. We make no claim that the four passers
identified below are useful descriptors; the screening verdict is
explicitly a redundancy diagnostic, not a chemical-utility
certification.
...
--- but the combined diagnostic that additionally requires
...
The four candidates that clear the combined diagnostic span three
...
\subsubsection{Comparison to LASSO / ElasticNet feature selection}\label{sub:lasso_comparator}

To position the pairwise / combined diagnostic against a generic
feature-selection comparator, we additionally fit LASSO (with
five-fold CV-tuned $\alpha$) and ElasticNet (with CV-tuned
$\alpha$ and $\ell_1$ ratio $\in \{0.3, 0.5, 0.7, 0.9\}$) on the
three regression datasets, using the same $30$-index baseline
matrix and the same $5$-fold cross-validation splits as
\texttt{full} / \texttt{pairwise\_pruned} / \texttt{combined\_pruned}.
The LASSO comparator is omitted on BBBP because a logistic-$\ell_1$
comparison would require regenerating the BBBP descriptor matrix,
which the present software environment cannot run end-to-end; the
BBBP gap is recorded in \S\ref{sec:limitations}.

\begin{table}[h]
\centering
\small
\caption{RandomForest screen-pipeline configurations vs.\ generic feature-selection (LASSO / ElasticNet) on the three regression datasets. RandomForest rows reproduce Table~\ref{tab:ml_benchmark}. LASSO and ElasticNet are linear models with CV-tuned regularisation, evaluated under the same 5-fold split protocol; their RMSE is reported here for comparison only.}
\label{tab:fs_comparison}
\begin{tabular}{llrr}
\toprule
Dataset & Configuration & RMSE & Mean $n$ features\\
\midrule
ESOL & \texttt{full} (RandomForest, 30 feats)         & $1.437$ & $30.0$\\
     & \texttt{pairwise\_pruned} (RandomForest)        & $1.435$ & $\phantom{0}7.4$\\
     & \texttt{combined\_pruned} (RandomForest)        & $1.475$ & $\phantom{0}4.4$\\
     & LASSO (linear, CV-tuned $\alpha$)              & $1.545$ & $14.6$\\
     & ElasticNet (linear, CV-tuned)                  & $1.546$ & $16.2$\\
\midrule
FreeSolv & \texttt{full} (RandomForest)               & $3.537$ & $30.0$\\
         & \texttt{pairwise\_pruned} (RandomForest)   & $3.540$ & $\phantom{0}8.0$\\
         & \texttt{combined\_pruned} (RandomForest)   & $3.561$ & $\phantom{0}3.2$\\
         & LASSO                                      & $3.527$ & $13.4$\\
         & ElasticNet                                 & $3.515$ & $18.6$\\
\midrule
Lipophilicity & \texttt{full} (RandomForest)          & $1.030$ & $30.0$\\
              & \texttt{pairwise\_pruned} (RandomForest) & $1.028$ & $\phantom{0}8.0$\\
              & \texttt{combined\_pruned} (RandomForest) & $1.287^{\,\dagger}$ & $\phantom{0}0.2$\\
              & LASSO                                 & $1.130$ & $15.6$\\
              & ElasticNet                            & $1.130$ & $17.8$\\
\bottomrule
\end{tabular}
...
The \texttt{pairwise\_pruned} screen matches or slightly beats the
LASSO / ElasticNet linear comparators on every regression dataset
while using roughly half the features ($7$--$8$ vs.\ $13$--$18$).
Note however that this comparison is across two model classes
(RandomForest for the screen-pipeline rows, linear models for
LASSO / ElasticNet); the comparison should be read as
``pairwise-pruned RandomForest is competitive with cardinal-feature
LASSO'' rather than as a head-to-head model-architecture benchmark.
Full per-fold numbers are in
...
\item \textbf{Alternative-family screening is a single-dataset case
study, not a chemical-utility claim.} The $20$-candidate experiment
of \S\ref{sec:novel_case} is on ESOL alone; cross-dataset
replication on FreeSolv, Lipophilicity, and BBBP is left as future
work. The four candidates that pass the combined diagnostic on
ESOL ($\mathrm{InfoH\_deg}$, $\mathrm{CutVerts}$,
$\mathrm{FourCyc}$, $\mathrm{MeanEcc}$) should be interpreted as
``residual-signal positive on $\log S$ specifically'', not as
useful QSPR descriptors.
...
\item \textbf{RandomForest is one model class; generic
feature-selection comparators are limited.}
Section~\ref{sub:lasso_comparator} compares the screen-pipeline
configurations against LASSO and ElasticNet on the three regression
datasets only; a classification-LASSO (Logistic $\ell_1$)
comparison on BBBP, and head-to-head benchmarks against
mRMR / Boruta, were not run for this paper and are noted as
follow-up work. The reported pairwise-pruned-vs-LASSO comparison
crosses model classes (RandomForest vs.\ linear), so it is a
sanity check, not a head-to-head model benchmark.
