...
benchmark (pipeline-then-LASSO vs.\ LASSO-only, same model class
throughout) replicates the parsimony claim without the
cross-model-class qualification: on every dataset the
pairwise-then-LASSO pipeline matches no-screen LASSO within
$5$-fold CV noise at roughly half the final LASSO-active features
...
The present paper includes a limited comparator benchmark against
LASSO, ElasticNet, mRMR, Boruta, and $\ell_1$-logistic feature
selection in Section~\ref{sub:lasso_comparator}. This comparison
is intended as a sanity check rather than a fully matched
end-to-end feature-selection study, because the screening-pipeline
rows use RandomForest whereas most external comparators are
evaluated with linear or logistic models. A fully matched
two-stage pipeline-versus-feature-selection benchmark remains
future work.
