...
(Table~\ref{tab:ml_benchmark}), and is competitive with several
generic feature-selection comparators (LASSO, ElasticNet, mRMR,
Boruta, $\ell_1$-logistic) under the reported mixed-model
benchmark (Table~\ref{tab:fs_comparison}). A matched-model-class
pipeline-then-LASSO / $\ell_1$-logistic benchmark supports the
same parsimony claim without the cross-model-class qualification
...
\centering
\small
\caption{Pipeline-then-LASSO matched-model-class benchmark. Same model class (LASSO for regression, $\ell_1$-Logistic for BBBP) and same 5-fold splits across all three configurations within each dataset, so the comparison isolates the effect of the upstream screening step. ``pre-LASSO feats'' is the mean number of features handed to LASSO after screening; ``final L1 feats'' is the mean number of features with non-zero LASSO coefficient (the screening step shrinks the input set; LASSO further shrinks via $\ell_1$ regularisation). Reported as mean $\pm$ std across the 5 folds. Full per-fold numbers in \texttt{results/paper2\_pipeline\_then\_lasso.csv}.}
\label{tab:matched_class}
\begin{tabular}{llcrr}
\toprule
Dataset & Configuration & Metric (mean $\pm$ std) & pre-LASSO feats & final L1 feats\\
\midrule
ESOL          & \texttt{full} (LASSO)                       & RMSE $1.536\pm 0.077$  & $30.0$         & $17.6$\\
              & \texttt{pairwise} $\to$ LASSO               & RMSE $1.580\pm 0.085$  & $\phantom{0}7.4$ & $\phantom{0}7.2$\\
              & \texttt{combined} $\to$ LASSO               & RMSE $1.588\pm 0.090$  & $\phantom{0}4.4$ & $\phantom{0}4.4$\\
\midrule
FreeSolv      & \texttt{full} (LASSO)                       & RMSE $3.469\pm 0.400$  & $30.0$         & $12.8$\\
              & \texttt{pairwise} $\to$ LASSO               & RMSE $3.465\pm 0.394$  & $\phantom{0}8.0$ & $\phantom{0}6.4$\\
              & \texttt{combined} $\to$ LASSO               & RMSE $3.462\pm 0.387$  & $\phantom{0}3.2$ & $\phantom{0}3.2$\\
\midrule
Lipophilicity & \texttt{full} (LASSO)                       & RMSE $1.128\pm 0.016$  & $30.0$         & $16.2$\\
              & \texttt{pairwise} $\to$ LASSO               & RMSE $1.137\pm 0.013$  & $\phantom{0}8.0$ & $\phantom{0}7.0$\\
...
BBBP          & \texttt{full} ($\ell_1$-Logistic)           & AUC $0.794\pm 0.019$   & $30.0$         & $11.6$\\
              & \texttt{pairwise} $\to$ $\ell_1$-Logistic   & AUC $0.787\pm 0.026$   & $\phantom{0}6.0$ & $\phantom{0}6.0$\\
              & \texttt{combined} $\to$ $\ell_1$-Logistic   & AUC $0.786\pm 0.028$   & $\phantom{0}4.0$ & $\phantom{0}4.0$\\
\bottomrule
\end{tabular}

\vspace{0.5em}
\noindent\small{$^\dagger$ Combined screen retains zero features in $4/5$ Lipophilicity folds; the value shown is from the single fold that retained one feature. The same failure mode appears in Table~\ref{tab:ml_benchmark} and is treated as a formal limitation in \S\ref{sub:limitations}.}
\end{table}

\textbf{Headline reading (matched-model-class).} Within each model
class, the pairwise-then-LASSO pipeline matches the no-screen
LASSO baseline within CV noise on every dataset while using
approximately half the final LASSO-active features: on ESOL
$\Delta\mathrm{RMSE}\approx +0.04$ at $7.2$ vs.\ $17.6$ active
features; on FreeSolv $\Delta\mathrm{RMSE}\approx -0.004$ at $6.4$
vs.\ $12.8$ features (a slight improvement); on Lipophilicity
$\Delta\mathrm{RMSE}\approx +0.01$ at $7.0$ vs.\ $16.2$ features;
on BBBP $\Delta\mathrm{AUC}\approx -0.007$ at $6.0$ vs.\ $11.6$
features. In every case the mean $\pm$ std intervals of
\texttt{full (LASSO)} and \texttt{pairwise}$\,\to\,$LASSO overlap.
The combined screen further halves the feature count on ESOL,
FreeSolv, and BBBP with a similar within-noise penalty, and
degenerates on Lipophilicity (same failure mode as the RF
benchmark; see \S\ref{sub:limitations}). The cross-model-class
qualification on Table~\ref{tab:fs_comparison} is therefore not
load-bearing: the screen-pipeline's parsimony claim survives a
fully matched same-model-class comparison.
...
$\ell_1$-logistic regression on BBBP: pairwise screening followed
by the same downstream model preserves performance within CV
noise while reducing the final active feature count. What remains
open is the broader matched comparison for other downstream
selectors and nonlinear / tree-ensemble models, such as
pipeline-then-XGBoost, pipeline-then-RandomForest with Boruta- or
mRMR-style selection, or nonlinear redundancy diagnostics.
\end{enumerate}
