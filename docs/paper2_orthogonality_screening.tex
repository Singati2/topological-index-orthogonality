...
classification) compares four feature configurations
(\texttt{full}, \texttt{pca\_95}, \texttt{pairwise\_pruned},
\texttt{combined\_pruned}) and confirms that the pairwise screen
matches the full feature set within CV noise on all four datasets
with roughly four-fold fewer features, while exposing an honest
failure mode of the strict combined screen on Lipophilicity
(Figure~\ref{fig:ml_benchmark}, Table~\ref{tab:ml_benchmark}). The
alternative-family results in this paper are for a single dataset
(ESOL, $\log S$); we do not generalize to other QSPR targets.
...
\subsection{Relation to the companion Wuzi paper, and why the split}\label{sec:relation_companion}
...
bounds in $(n, m, \Delta, \delta)$, bounds via classical indices
(including a uniform ratio-bound theorem and its specialisations
to $R$, $\chi$, and $SO$), extremal-graph analysis, and the
sensitivity-analysis benchmarks on $18$ octanes, $106$ trees of
order $10$, and $75$ decane isomers --- are developed in a
companion graph-theory paper~\cite{Paper1_Wuzi} aimed at a
mathematical-chemistry / MATCH-style audience. The present paper
is focused on the screening pipeline as a reusable methodology;
it uses the Wuzi family only as one screened candidate
(Section~\ref{sec:wuzi_case}) alongside the twenty
alternative-family candidates (Section~\ref{sec:novel_case}) and
re-derives no Wuzi-specific mathematical results.

We have organised the work as two papers rather than one because
the two have non-overlapping audiences and non-overlapping
contributions: the graph-theory paper contributes new bounds,
extremal conjectures, and parametric closed forms that are
on-topic for math-chem venues and off-topic for a methodology
venue; the methodology paper contributes a multi-diagnostic
screening pipeline, the structural Edge-Degree-Pair Basis
observation in operational form, and a downstream cross-validated
ML benchmark across four QSPR datasets that are on-topic for a
methodology venue and off-topic for a math-chem one. The two
papers cite each other but neither requires reading the other to
follow the contributions. A reader interested only in the
screening methodology (the present paper) can take the Wuzi family
as a black-box parametric BID family throughout.
...
\subsection{Structural interpretation of the four passers}\label{sec:passer_analysis}
...
Wiley-VCH, Weinheim, 2009 (originally \emph{Handbook of Molecular
Descriptors}, Wiley-VCH, 2000).
(Full publisher / page-range details to be confirmed at submission.)
