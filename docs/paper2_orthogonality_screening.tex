...
exhibits $I_f$ as a linear combination of the $G$-dependent counts
$\{m_{ij}(G)\}_{i\le j}$ with coefficients $\{f(i,j)\}_{i\le j}$
depending only on $f$. The linear map
$\bid\to\mathbb{R}^{N_\Delta}$ sending $I_f$ to the vector
$(f(i,j))_{i\le j}$ is well defined and injective, where
$N_\Delta = |\{(i,j):1\le i\le j\le\Delta\}|=\binom{\Delta+1}{2}$
by the standard multiset-counting identity. Hence
$\dim\bid\le\binom{\Delta+1}{2}$; for $\Delta=4$ this is
$\binom{5}{2}=10$.
\end{proof}
...
methodological point: \emph{passing the orthogonality screen is
a necessary but not sufficient condition for QSPR usefulness}.
The Wuzi FreeSolv borderline point illustrates the failure mode
sharply: a parameter setting that clears the $|r| < 0.95$
threshold can still have negligible partial correlation with the
target, as the partial-correlation diagnostic of
Section~\ref{sec:protocol} catches. The combined criterion
(Table~\ref{tab:decision}) is more faithful to the QSPR-utility
question than the bare pairwise correlation threshold.

The downstream ML benchmark (Section~\ref{sec:ml_benchmark})
extends the methodological point to predictive performance. The
pairwise screen reduces feature count by roughly $4\times$ on all
four MoleculeNet datasets while matching the full-baseline
RandomForest within cross-validation noise. The strict combined
screen is more selective and, in the limit case of Lipophilicity,
retains zero features in $4$ of $5$ folds --- an honest failure
mode that reflects the weakness of the topology-to-$\log D$
partial-correlation structure on that dataset. The combined
screen should therefore be read as a diagnostic rather than as a
hard filter: it flags the exact failure mode the Wuzi family at
$(-1, -1, 1)$ exhibits, but its use as a feature-selection
criterion needs to be tied to the QSPR utility loss the analyst
is willing to tolerate.

The Edge-Degree-Pair Basis (Theorem~\ref{thm:bid}) gives the
structural reason that parametric BID families on
hydrogen-suppressed molecular graphs cannot escape the empirical
redundancy ceiling reported across the three datasets: the
$\binom{5}{2} = 10$-dimensional cap is intrinsic to the BID
construction. The methodology paper and the companion graph-theory
paper~\cite{Paper1_Wuzi} together close the loop: the graph-theory
paper studies the family's mathematical structure, and the present
paper studies its statistical structure relative to a fixed
baseline.
