...
hundreds of scalar topological indices, and the conventional
pairwise-correlation criterion for ``does a new index carry
independent information''
($\max_{b}\lvert r(z, X_b)\rvert < 0.95$ against a classical
baseline) is empirically over-permissive.

\textbf{Method.} We present an open-source linear diagnostic
pipeline combining pairwise correlation, PCA effective rank, VIF,
and partial correlation with the QSPR target into a \emph{combined
diagnostic} (Table~\ref{tab:decision}), positioned as a
pre-flight screen for independent linear QSPR signal, not as a
hard feature-selection filter or a utility certification. The
\emph{Edge-Degree-Pair Basis} (Theorem~\ref{thm:bid}) gives a
complementary structural ceiling: every bond-incident-degree (BID)
index on hydrogen-suppressed molecular graphs with $\Delta \le 4$
lies in a vector space of dimension at most $10$.

\textbf{Evidence.} The three-parameter Wuzi index family
\cite{Paper1_Wuzi} fails the combined diagnostic at every one of
$300$ grid points across three regression datasets, with the
verdict stable across nine $(\tau_r, \tau_{\mathrm{pcor}})$
threshold combinations (\S\ref{sub:threshold_sensitivity}). Across
all four datasets, $15$--$17$ of $20$ alternative-family
candidates clear pairwise but only $0$--$4$ also clear
$\lvert\mathrm{pcor}\rvert \ge 0.10$
(Table~\ref{tab:novel_combined}). A $5$-fold-CV RandomForest
benchmark shows the pairwise-pruned set preserves predictive
performance within CV noise at $\sim 4\times$ fewer features
(Table~\ref{tab:ml_benchmark}); a matched-model-class
pipeline-then-LASSO / $\ell_1$-logistic benchmark with paired
$t$-tests shows a clean parsimony win on FreeSolv and BBBP
($p > 0.05$) and a small but statistically detectable
$<3\%$ RMSE cost on ESOL and Lipophilicity in exchange for
${\sim}\!2\times$ fewer LASSO-active features
(Table~\ref{tab:matched_class}).

\textbf{Scope.} Diagnostics are linear; the BID theorem covers
$\Delta\le 4$; the combined screen degenerates on Lipophilicity
(zero features in $4/5$ folds) and should not be used as a hard
filter. Full limitations in \S\ref{sub:limitations}.
...
\begin{table}[h]
\centering
\caption{Screening diagnostic summary. Satisfying both conditions is interpreted as evidence of non-negligible residual linear signal under the tested baseline, dataset, and thresholds; it is not a proof of QSPR utility.}
\label{tab:decision}
\begin{tabular}{lll}
\toprule
...
RandomForest predictive performance within CV noise at roughly
$4\times$ fewer features; in a matched-model-class
pipeline-then-LASSO benchmark (Table~\ref{tab:matched_class}),
parsimony at ${\sim}\!2\times$ fewer LASSO-active features comes at
no statistically detectable cost on $2$ of the $4$ datasets and at
$<3\%$ RMSE cost on the other $2$. The Edge-Degree-Pair Basis (Theorem~\ref{thm:bid})
