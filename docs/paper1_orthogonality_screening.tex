...
Lipophilicity & $\log D$ at pH 7.4 & $4\,200$ & \cite{Wang2009ChEMBL} via \cite{MoleculeNet}\\
\bottomrule
\end{tabular}
\end{table}
...
At the single borderline grid point on FreeSolv
($(\alpha,\beta,\gamma)=(-1,-1,1)$, $\max|r|=0.943$ with $mM_2$),
the Wuzi value is exactly captured by a linear combination of the
$30$ baseline indices --- the residual after regressing the Wuzi
value on the baseline has variance below numerical precision ---
so the partial correlation with $\Delta G_{\mathrm{hyd}}$ is
identically zero. The maximum $|\mathrm{pcor}|$ across all $100$
FreeSolv grid points (achieved at a different non-passing point) is
$0.036$. Either way the FreeSolv ``pass'' fails the combined
criterion of Table~\ref{tab:decision} and is classified as not
adding meaningful QSPR signal. This example illustrates the
...
\bibitem{Wang2009ChEMBL}
\todo{[LIPOPHILICITY DATASET CITATION REFERENCE NEEDED]}.
