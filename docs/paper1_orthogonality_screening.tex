...
% Paper 1: Orthogonality Screening of Topological Indices for QSPR Modeling
% Compile with: pdflatex paper1_orthogonality_screening.tex (twice for cross-refs)
...
% \todo{...} markers are kept in the source but suppressed in PDF.
% Restore by swapping for: \newcommand{\todo}[1]{\textcolor{red!70!black}{\textbf{[#1]}}}
\newcommand{\todo}[1]{}
...
\author{
  Ganesh Shiwakoti\thanks{Department of Biostatistics, Florida Atlantic University, USA. \texttt{mpcrlab@gmail.com}} \and
...
\subsection{Relation to Paper 2}
This paper presents the methodology. The companion mathematical
paper~\cite{Paper2_Wuzi} develops the Wuzi index family in detail
(closed forms, bounds, extremal graphs). The two papers share the
numerical work in Sections~\ref{sec:wuzi_case}--\ref{sec:novel_case}
of this manuscript with the corresponding Numerical work section
of~\cite{Paper2_Wuzi}.
...
The Wuzi index family~\cite{Paper2_Wuzi} is defined as
\[
W(G;\alpha,\beta,\gamma)=\sum_{uv\in E(G)}
(d_u d_v)^\alpha(d_u+d_v)^\beta
\exp\!\left(\gamma\,\frac{|d_u-d_v|}{d_u+d_v}\right),
\]
a three-parameter generalization that contains $M_2$, Randi\'c,
sum-connectivity, and harmonic as special cases. We screen the
family on a $100$-point grid
$(\alpha,\beta,\gamma)\in\{-1,-0.5,0,0.5,1\}^2\times\{0,0.5,1,2\}$
on each of the three QSPR datasets.
...
The full per-grid-point numerical workup of the Wuzi family ---
$100$ grid points per dataset, $300$ grid points across the three
datasets --- is reported in the Numerical work section of the
companion mathematical paper~\cite{Paper2_Wuzi}. We reproduce the
compact verdict in Table~\ref{tab:wuzi_verdict_paper1} so that the
present paper is self-contained, and isolate the parameter triple
$(\alpha, \beta, \gamma) = (-1, -1, 1)$ because it is
methodologically central for the present paper.
...
datasets. Reproduced from~\cite{Paper2_Wuzi} for the convenience of
...
datasets under four feature configurations, and report whether the
screened feature set matches the full-baseline model's predictive
performance.
...
\textbf{BBBP} (Martins~2012 via~\cite{MoleculeNet}),
$n = 2050$ molecules, binary target ``blood-brain barrier
penetration''. The class distribution is approximately $3{:}1$
(positive:negative), handled with a class-balanced RandomForest.

\textbf{Configurations.} An identical $80/20$ split (stratified for
the classification task) is used across all configurations so the
comparison isolates the effect of feature selection.
\begin{itemize}
\item \textbf{full}: all $30$ baseline indices.
\item \textbf{pca\_95}: top-$k$ principal components of the
standardized baseline at $95\%$ cumulative variance ($k=3$ on all
four datasets).
\item \textbf{pairwise\_pruned}: greedy redundancy filter at
$|r| < 0.95$ (keep one representative per correlation cluster).
\item \textbf{combined\_pruned}: pairwise filter plus
$|\mathrm{pcor}(z, y \mid X)| \ge 0.10$ on the pairwise-pruned set.
\end{itemize}

\textbf{Results.} Table~\ref{tab:ml_benchmark} and
Figure~\ref{fig:ml_benchmark} summarize the per-dataset, per-configuration
RandomForest performance and feature count.

\begin{table}[h]
\centering
\caption{RandomForest test-set performance under four feature configurations. Regression metric is RMSE (lower is better); classification metric is ROC-AUC (higher is better). Identical splits across configurations.}
\label{tab:ml_benchmark}
\begin{tabular}{llrrrrrr}
\toprule
Dataset & Task & full & pca\_95 & pairwise\_pruned & combined\_pruned & full $n_{\mathrm{feat}}$ & pruned $n_{\mathrm{feat}}$\\
\midrule
ESOL          & regression       & 1.454 & 1.503 & 1.445 & 1.508 & 30 & 7\\
FreeSolv      & regression       & 3.890 & 3.896 & 3.875 & 3.963 & 30 & 8\\
Lipophilicity & regression       & 1.037 & 1.064 & 1.037 & ---   & 30 & 8\\
BBBP          & classification   & 0.835 & 0.813 & 0.864 & 0.859 & 30 & 6\\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Downstream ML benchmark on four datasets. Top row: primary
metric (RMSE for the three regression panels, ROC-AUC for BBBP).
Bottom row: number of features in each configuration. The pairwise
screen matches or marginally improves the full-baseline RandomForest
on every dataset while using $4$--$5\times$ fewer features. On
Lipophilicity the strict combined screen retains zero features
(no pairwise-pruned index has $|\mathrm{pcor}| \ge 0.10$ with $\log D$),
which is reported as a failure mode rather than hidden.}
\label{fig:ml_benchmark}
\end{figure}

\textbf{Interpretation.} The \texttt{pairwise\_pruned} configuration
matches \texttt{full} within $0.02$ RMSE on every regression dataset
and marginally improves ROC-AUC on BBBP ($0.835 \to 0.864$), using
$6$--$8$ features rather than $30$. This is the methodology paper's
operational claim: redundancy screening at $|r| < 0.95$ preserves
predictive performance while substantially reducing the input
dimension. The stricter \texttt{combined\_pruned} configuration
trades some performance for more aggressive feature reduction; on
Lipophilicity it removes all features, an honest failure mode that
reflects the dataset's weak topology-to-$\log D$ partial-correlation
structure (no pairwise-pruned index has $|\mathrm{pcor}| \ge 0.10$).
The threshold $|\mathrm{pcor}| = 0.10$ should be relaxed (or replaced
...
\emph{The Wuzi Index Family: Graph-Theoretic Properties, Bounds, Extremal Graphs, and Redundancy Analysis},
in preparation, 2026.
