...
The full numerical workup of the Wuzi family --- $100$ grid points
per dataset, $300$ grid points across the three datasets, with per-
grid-point $\max|r|$ and partial-correlation values --- is reported
in the companion mathematical paper~\cite{Paper2_Wuzi},
Section~6.2 (Table~6.2 and Figure~5b therein). We summarize the
verdict and isolate the parameter triple $(\alpha, \beta, \gamma) = (-1, -1, 1)$
because it is methodologically central for the present paper.
...
the maximum partial correlation with the target across all $300$
grid points is $0.036$ (FreeSolv) and below $0.10$ on every grid
point on every dataset, so by the combined criterion of
