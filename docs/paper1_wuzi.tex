...
On the hydrogen-suppressed molecular graphs $\mathcal{G}_4$, the
ratio extrema $c_{\min}^{h}, c_{\max}^{h}$ are finite-set extrema
over the ten admissible degree pairs $\{(i, j) : 1 \le i \le j \le 4\}$
and are computable by brute force given any parameter triple
$(\alpha, \beta, \gamma)$. For each classical index $I_h$ in
$\{M_1, M_2, R, SO, GA, H, ABC\}$ (with $ABC$ restricted to graphs
without a $(1, 1)$ edge, since $h_{ABC}(1, 1) = 0$), the
