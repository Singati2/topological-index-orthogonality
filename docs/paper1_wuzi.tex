...
(Theorem~\ref{thm:m1_bound}), and a rigorous ratio-bound theorem
that applies uniformly to every classical BID index with
strictly positive edge contribution (Theorem~\ref{thm:ratio_bound}).
...
The graph-specific extrema in Theorem~\ref{thm:ratio_bound} can be
replaced by uniform $\mathcal{G}_4$-wide extrema at the cost of a
looser (but $G$-independent) bound: on hydrogen-suppressed molecular
graphs $\mathcal{G}_4$, the ratio extrema $c_{\min}^{h}, c_{\max}^{h}$
are finite-set extrema over the ten admissible degree pairs
$\{(i, j) : 1 \le i \le j \le 4\}$ and are computable by brute force
given any parameter triple $(\alpha, \beta, \gamma)$. More generally,
whenever $h$ vanishes on some admissible pair (for example
$h_{ABC}(1, 1) = 0$), $c_{\max}^{h}$ becomes infinite on any graph
that realises that pair and the upper ratio bound is vacuous; the
hypothesis ``$h(i, j) > 0$ on every pair appearing in $G$'' in
Theorem~\ref{thm:ratio_bound} is the correct restriction. For each
classical index $I_h$ in $\{M_1, M_2, R, SO, GA, H, ABC\}$ (the $ABC$
case with the natural $(1,1)$-free restriction), the
script \texttt{scripts/09\_wuzi\_bounds\_tables.py} tabulates
...
\begin{theorem}[Bound via generalized $R$ and $\chi$ -- skeleton]\label{thm:R_chi}
\todo{PROOF SKETCH -- TO BE COMPLETED}
Let $\gamma = 0$. Define the generalized Randi\'c and
sum-connectivity indices by
$R_{a}(G) = \sum_{uv \in E(G)} (d_u d_v)^{a}$ and
$\chi_{b}(G) = \sum_{uv \in E(G)} (d_u + d_v)^{b}$, so that the
classical cases are $R = R_{-1/2}$ and $\chi = \chi_{-1/2}$.
Applying Theorem~\ref{thm:cs} with the split $(\alpha_1, \beta_1, 0)
= (2\alpha, 0, 0)$ and $(\alpha_2, \beta_2, 0) = (0, 2\beta, 0)$
(so that $\alpha_1 + \alpha_2 = 2\alpha$ and
$\beta_1 + \beta_2 = 2\beta$),
\[W(G;\alpha,\beta,0)\;\le\; \sqrt{R_{2\alpha}(G)\cdot \chi_{2\beta}(G)},\]
the right-hand side being a product of generalized classical
indices. For the classical case
$(\alpha, \beta) = (-1/4, -1/4)$ this specialises to
$W \le \sqrt{R \cdot \chi}$.
\end{theorem}
